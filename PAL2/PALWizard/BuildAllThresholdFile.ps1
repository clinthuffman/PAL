...
'<?xml version="1.0"?>' | Set-Content .\All.xml -Encoding Ascii
'<PAL PALVERSION="2.0" NAME="All" DESCRIPTION="This runs the counter log through all of the threshold files." CONTENTOWNERS="Clint Huffman" FEEDBACKEMAILADDRESS="clinth@microsoft.com" VERSION="0.1" LANGUAGE="English" LANGUAGECODE="en">' | Add-Content .\All.xml -Encoding Ascii


$oFiles = Get-ChildItem -Path $WorkingDirectory\*.xml

$aExceptions = @('All.xml','Custom.xml','PALFunctions.xml','CalculatedIops.xml')
    ...
    If ($IsException -eq $false)
    {
        ...
        ForEach ($sLine in $oLines)
        {
            ...
            If (($IsPalThresholdFile -eq $True) -and ($IsAdded -eq $false))
            {
                ...
                $sLine | Add-Content .\All.xml -Encoding Ascii
            }
        }
    }
}
'</PAL>' | Add-Content .\All.xml -Encoding Ascii
